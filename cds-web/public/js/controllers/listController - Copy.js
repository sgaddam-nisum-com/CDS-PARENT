...
define(['controllers/controllerModule'], function (controllerModule) {
	 ...
	 controllerModule.controller('listController', ["$http",'$scope',"$state","listService", function($http, $scope,$state,listService){
		...
		this.search = function(){
			...
			listService.getUserList({ q: this.searchQ, page:1 }, function(resObj) { 
				that.userList = resObj.data.searchResults;
		...
		this.editUserInfo = function(citizenId){
				$state.go('root.profile.editprofile.personal', {'citizenId': citizenId});
		}
